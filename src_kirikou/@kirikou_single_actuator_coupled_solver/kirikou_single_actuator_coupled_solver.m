classdef kirikou_single_actuator_coupled_solver < handle
    ...
    methods
        function DSD = kirikou_single_actuator_coupled_solver(Ct, ipc)
        ...
        function preprocess_inputs(DSD)
            ...
            % -> New: Scalable
            % DSD.f1_average  =   F1 / (DSD.d1 * DSD.rho);                    % Average Actuator Force per unit radius, divided by density (== specific enthalpy jump)
            % DSD.f1_average =   (DSD.u_inf) * F1 / (DSD.d1 * DSD.rho);     % Average Actuator Force per unit radius, divided by density (== specific enthalpy jump), scaled to speed (get things into circulation)
            DSD.f1_average =   F1 / DSD.d1;                                % Average Actuator Force per unit radius
            % >> End of edition
        ...
        function define_discretization_and_initial_geometry(DSD)
            ...
            disp(['Wake Lenght : ' num2str(max(x)) ' = ' num2str(max(x)/DSD.d1) ' diameters']);
            ...
            % -> New: Scalable
            delta_f_w_side = (DSD.f_w(y_over_r_side_outer) - DSD.f_w(y_over_r_side_inner)) * (DSD.u_inf/DSD.rho);
            % >> End of edition
...
%             DSD.gamma0 = DSD.gamma0(:);
        ...
        function run_solver(DSD)
            ...
            for n=1:DSD.N_iterations
                ...
                dx_center = DSD.t_n.* DSD.VS.x_n_unit_vector * (DSD.dtx/DSD.u_inf);
                dy_center = DSD.t_n.* DSD.VS.y_n_unit_vector * (DSD.dt /DSD.u_inf);
                ...
                % Update gamma
                DSD.gamma               = DSD.gamma - (DSD.relax/DSD.u_inf) * RESl_stretch ./ RESl_stretch_dgamma;
        ...
        function compute_machine_parameters(DSD)            
            ...
            % -> New: Scalable
            f1_actual_patches   = - cumsum(DSD.delta_f_w .* sign(DSD.y_release)) * (DSD.rho / DSD.u_inf);
            % >> End of edition
            ...
            [~ , u_VS_actuator1    , ~]   = induced_speed_on_many_points(DSD.VS , x_range1 , y_range1);
            [    u_ipc_actuator1   , ~]   = induced_speed_on_many_points(DSD.ipc, x_range1 , y_range1);
            ...
            % -> New, scalable:
            ua_num = trapz(y_range1, u_a1) / trapz(y_range1, ones(size(y_range1)));
            % >> End of edition
            a1_num = DSD.a1;
            ...
            % This does not work in adim space : C_F_b = F_b / (1/2 * DSD.rho * DSD.u_inf^2 * DSD.S1);
            % This works in adim space : C_F_b = F_b;
            % Try this:
            ...
            ua_theo     = DSD.u_inf * (1/2 * (1 + ue_fun(C_F_a)) - 1/2* C_F_b / (ue_fun(C_F_a) - 1));
