function [px_air_morphed, py_air_morphed, px_air_scaled, py_air_scaled] = fuse_airfoil_coordinates(BPO_upper, BPO_lower, airfoil_file, eta_airfoil, flip_airfoil)
% fuse_airfoil_coordinates() - Fusing function(2nd generation morphing) of
% airfoil shapes for Donqi (or other!) ducts
% 
% Ordered inputs (with typical values): 
%   BPO_upper       = 16;                      % Number of polynomial parameters (order=degree+1) for description of airfoil upper (suction ) side shape
%   BPO_lower       = 16;                      % Number of polynomial parameters (order=degree+1) for description of airfoil lower (pressure) side shape
%   airfoil_file    = 'airfoils/donqio.air';   % Filename of reference airfoil
%   eta_airfoil     = 0 ;                      % Fusing parameter (-1 = thin plate variant, 1 = symmetric variant, 0 = original, and everything in between!)
%   flip_airfoil    = true;                    % If true airfoil is flipped upside down (boolean!) (post-morphing)
% Ordered outputs :
%   px_air_morphed  - Chordwise (x/c) coordinates of fused (2nd generation morphing) airfoil
%   py_air_morphed  - Thickness (t/c) coordinates of fused (2nd generation morphing) airfoil
%   px_air_scaled   - Chordwise (x/c) coordinates of reference airfoil (after setting longest internal line parallel to x axis, scaling resulting chord to unity and translating LE to origin)
%   py_air_scaled   - Thickness (t/c) coordinates of reference airfoil (after setting longest internal line parallel to x axis, scaling resulting chord to unity and translating LE to origin)
%
% Requirements:
%
% Built for and upon:
...
[~, ~, ~, parameters]= get_parameters_from_file(SF , airfoil_file);
