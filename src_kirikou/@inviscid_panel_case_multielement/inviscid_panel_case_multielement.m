classdef inviscid_panel_case_multielement < handle
    ...
    properties
        ...
        CF_x                    % Force coefficient in X direction
        CF_y                    % Force coefficient in Y direction
    end
...
%             u_v_t_column = sum(ipc.u_v_t_M,2);
            ...
            % From pressures
            % Define Cp
            ipc.cp = 0.5 * ipc.rho * ipc.u_t_vec.^2;
            ...
            % Obtain Net Force Coefficients
            ipc.CF_x = ipc.F_x ./ (0.5 * ipc.rho * ipc.u_inf^2 * ipc.c);
            ipc.CF_y = ipc.F_y ./ (0.5 * ipc.rho * ipc.u_inf^2 * ipc.c);
