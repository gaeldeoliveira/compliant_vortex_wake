classdef induction_functions
    ...
    methods(Static)
        ...
        function [u , v] = constant_strenght_singular_source_pair(x, y, x_v, y_v, gamma_v)
            % Two Dimensional Constant Strenght Singular Vortex Pair Induction Function
            %
            % Inputs:
            %    x  ,   y      % Target
            %    x_v,   y_v    % Upper Halfplane Singular Source Center
            %    x_v, - y_v    % Lower Halftplane Singular Source Center
            %    gamma_v       % Strenght of Upper Singular Source
            %    gamma_v       % Strenght of Lower Singular Source
            ...
            %\ % Lifting Vortex Helper Functions and Variables
            r_sq_v_fun = @(x,y,x_v,y_v) (x-x_v).^2 + (y-y_v).^2;
            u_v_fun = @(x,y,x_v,y_v, gamma_v) gamma_v / (2*pi()) * (  x-x_v ) ./ r_sq_v_fun(x,y,x_v,y_v);
            v_v_fun = @(x,y,x_v,y_v, gamma_v) gamma_v / (2*pi()) * (  y-y_v ) ./ r_sq_v_fun(x,y,x_v,y_v);
