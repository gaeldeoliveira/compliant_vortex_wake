classdef kirikou_single_actuator_solver < handle
    ...
    methods
        ...
        function preprocess_inputs(DSD)
            ...
            % Set force from Ct
            F1             = - 0.5*DSD.rho*DSD.S1*(DSD.u_inf^2)*DSD.Ct;   % Total actuator Force 
            % << Old: Non scalable
            %DSD.f1_average =   F1 / DSD.d1;                              % Average Actuator Force per unit radius
            % -> New: Scalable
            DSD.f1_average =    F1 / (DSD.d1);     % Average Actuator Force per unit radius, divided by density (== specific enthalpy jump), scaled to speed (get things into circulation)
            % >> End of edition
        ...
        function define_discretization_and_initial_geometry(DSD)
            ...
            % Determine force step for each filament (proportional to vorticity generation rate: force derivative in radial direction, scaled with u_inf over rho)
            % << Old: Non scalable
