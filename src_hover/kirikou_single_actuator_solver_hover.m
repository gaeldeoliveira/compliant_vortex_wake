classdef kirikou_single_actuator_solver_hover < handle
    ...
    methods
        function DSD = kirikou_single_actuator_solver_hover(Ct, x_v, r_v, gamma_v)
...
%             DSD.gamma0 = DSD.gamma0(:);
        ...
        function formulate_initial_guesses(DSD)
            ...
            DSD.s_l = zeros(DSD.n_panels*DSD.n_filaments, 1);
        ...
        function run_solver(DSD)
            ...
            for n=1:DSD.N_iterations
                ...
                % Version for hover (rotation method, constant panel
                % lenght)
                %   Find local flow misalignment angle (in rad)
                tet_misalign  = atan2(DSD.t_n, DSD.s_l);
                %   Choose angle shift of current step
                dtet          = - tet_misalign * DSD.dt;
                %   Find vector of each panel
                x_end_m_start = DSD.x_end - DSD.x_start; % In principle equal to: DSD.VS.l .* DSD.VS.x_l_unit_vector
                y_end_m_start = DSD.y_end - DSD.y_start; % In principle equal to: DSD.VS.l .* DSD.VS.y_l_unit_vector
                %   Rotate panel into that direction
                dx_end =  cos(dtet) .* x_end_m_start + sin(dtet) .* y_end_m_start - x_end_m_start;
                dy_end = -sin(dtet) .* x_end_m_start + cos(dtet) .* y_end_m_start - y_end_m_start;
%                 dy_end =
%                 %   Find local flow misalignment
%                 DSD.VS
                dx_center = dx_end /2;
                dy_center = dy_end /2;
                % tet1 = atan2(DSD.VS.y_l_unit_vector+dy_canonic, DSD.VS.x_l_unit_vector);
...
%             DSD.Cp1  = DSD.p1 / (0.5*DSD.rho*DSD.S2*(DSD.u_inf^3));
        ...
        function [a, m, p, u_section] = compute_induction_on_cross_section(DSD, x_section, r_section, f_section)
            % Find Section Diameter and Surface
            d_section = 2*r_section;            
            % Discretize the Cross-Section
            y_range = linspace(-(r_section-eps), (r_section-eps), 10000);
