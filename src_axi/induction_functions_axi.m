classdef induction_functions_axi
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %                                                                     %
    %   Update  :   Get axi-symmetric induction functions in there!       %
    %   Date    :   October 2017 to July 2018                             %
    %                                                                     %
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    methods(Static)
        function [u , v] = constant_strenght_singular_vortex_pair_2d(x, y, x_v, y_v, gamma_v)
        ...
        function [u , v] = constant_strenght_singular_vortex_ring_axi(x, y, x_v, y_v, gamma_v)
            % Induction of single vortex ring on many point
            
            % Parse constants
            delta             = induction_functions_axi.delta;
            ellipke_tol       = induction_functions_axi.ellipke_tol;
            
            % Compute
            [u_z_mesh, u_r_mesh, ~] = vortex_ring_induction_alfa6_fun(-gamma_v, x_v, y_v, delta, ellipke_tol, x, y);
        ...
        function [u , v] = constant_strenght_singular_vortex_pair(x, y, x_v, y_v, gamma_v)
            % Planar Mode       (deactivated  )
            % [u , v] = induction_functions.constant_strenght_singular_vortex_pair_2d(x, y, x_v, y_v, gamma_v);
            
            % Axisymmetric Mode (activated)
            [u , v] = induction_functions_axi.constant_strenght_singular_vortex_ring_axi(x, y, x_v, y_v, gamma_v);
        end
        
        function [u , v] = constant_strenght_singular_source_ring_axi(x, y, x_v, y_v, gamma_v)
            % Induction of single vortex ring on many point
            ...
            % Compute
            [u_z_mesh, u_r_mesh, ~] = source_ring_induction_alfa6_fun(-gamma_v, x_v, y_v, delta, ellipke_tol, x, y);
        ...
        function [potential , u , v] = constant_strenght_vortex_segment_element_axi(x, y, x1, y1, x2, y2, gamma)
            % Axisymmetric (virtual) Constant Strenght Singularity Segment Elements
            % Constructed with K (typical =128) singular vortices (vortex rings)
            
            % Parse constants
            delta                   = induction_functions_axi.delta;
            ellipke_tol             = induction_functions_axi.ellipke_tol;
            K                       = induction_functions_axi.K;
            % Compose inputs
        ...
        function [potential , u , v] = constant_strenght_vortex_segment_element(x, y, x1, y1, x2, y2, gamma)
            ...
            % Axisymmetric Mode (activated)
            [potential , u , v] = induction_functions_axi.constant_strenght_vortex_segment_element_axi(x, y, x1, y1, x2, y2, gamma);
        end
        
        function [potential , u , v] = constant_strenght_source_segment_element_axi(x, y, x1, y1, x2, y2, gamma)
            % Axisymmetric (virtual) Constant Strenght Singularity Segment Elements
            % Constructed with K (typical =128) singular vortices (vortex rings)
            ...
            l                       = sqrt((x2-x1).^2 + (y2-y1).^2);
            % Compute
            [u_z_mesh, u_r_mesh, ~] = source_line_induction_alfa6_fun(-gamma * l, x1, y1, x2, y2, delta, ellipke_tol, K, x, y);
            % Recast outputs
            potential = zeros(size(u_z_mesh));
            u = u_z_mesh;
            v = u_r_mesh;
        ...
        % Deprecated (pure 2d, no axi clone for now)
