...
%       d     = 1     m         Diameter
Ct           =  0.65;     % [-- ] Actuator Force Coefficient (C_F_a)
...
x_duct       = -0.05;           % Translation of airfoil rotation point from origin (x direction)
y_duct       =  0.60;           % Translation of airfoil rotation point from origin (y direction)
c_duct       =  0.30;           % Chord of of duct airfoil, as ratio of input file  (assumed to have chord 1)
...
addpath src_vortex_ring_induction/
...
%% Panel Case Creation, Definition and Solution (Planar Flow)
% Create Case Object
...
%% Create Solver Object (Planar Flow)
DSD = kirikou_single_actuator_coupled_solver(Ct, ipc);

% Put this in monitoring mode
DSD.information_period = 400;
DSD.n_stances          =  60;
% Reduce wake lenght for consistency with axi case
DSD.theta_end           = 89 / 180 * pi(); % 88 is 14D, 89 is 28D
% Relax convergence criteria a bit for speed
DSD.max_RES_stretch    = 1e-9;                        % Maximum Stretching (extension) residual (vorticity conservation)
DSD.max_RES_shape      = 1e-9;                        % Maximum Shape residual (volume flow accross segments)
...
%% Panel Case Creation, Definition and Solution (Planar Flow)
% Create Case Object
ipc_axi          = inviscid_panel_case_multielement_axi([px_top] , [py_top], [1], [length(px_top)]); %#ok<NBRAK>
% Describe Straight Inflow
ipc_axi.u_inf    = u_inf;
ipc_axi.alpha    = alpha;
% Describe Still-Air Rotation
ipc_axi.c_over_R = c_over_R;
ipc_axi.x_cp     = x_cp;
ipc_axi.rotation = rotation;
% Force, Solve and Postprocess
ipc_axi.generate_solution;



%% Create Solver Object (Axisymmetric Wake Flow, with Planar Body)
DSD_axi = kirikou_single_actuator_coupled_solver_axi(Ct, ipc_axi);

% Put this in monitoring mode
DSD_axi.information_period = 400;
DSD_axi.n_stances          =  60;
% Reduce wake lenght to avoid issues
DSD_axi.theta_end           = 89 / 180 * pi(); % 88 is 14D, 89 is 28D
% Relax convergence criteria a bit for speed
DSD_axi.max_RES_stretch    = 1e-9;                        % Maximum Stretching (extension) residual (vorticity conservation)
DSD_axi.max_RES_shape      = 1e-9;                        % Maximum Shape residual (volume flow accross segments)

% Prepocess Inputs
DSD_axi.preprocess_inputs();
% Discretize with Straight Wake Tube as Initial Guess
DSD_axi.define_discretization_and_initial_geometry();
% Define Reference Panel Strenghts
DSD_axi.define_reference_panel_strenghts();
% Now formulate Initial Guesses
DSD_axi.formulate_initial_guesses();
% Create Vortex Element Object (with state provided by initial guesses)
DSD_axi.create_vortex_element_object();
% Now Start Solver
DSD_axi.run_solver();
% Now compute Machine Parameters (still some issues here)
DSD_axi.compute_machine_parameters();

%% Save State
save kirikou_example_case_02

%% Extract element solutions
...
% Axi (with body)
plot(DSD_axi.x_start(DSD_axi.id_start(1):DSD_axi.id_end(1)), DSD_axi.y_start(DSD_axi.id_start(1):DSD_axi.id_end(1)), 'x-' , 'Color', [0.8500 0.3250 0.0980]); hold on
plot(DSD_axi.x_start(DSD_axi.id_start(2):DSD_axi.id_end(2)), DSD_axi.y_start(DSD_axi.id_start(2):DSD_axi.id_end(2)), 'x-' , 'Color', [0.8500 0.3250 0.0980]); 
% And plot body
plot(element(1).px_middle , element(1).py_middle, '-' , 'Color', [0 0 0]);
plot(element(2).px_middle , element(2).py_middle, '-' , 'Color', [0 0 0]);
% Bells and Whistles
title('Wake Shape')
legend('Upper Side - Planar', 'Lower Side - Planar', 'Upper Side - Axisymmetric', 'Lower Side - Axisymmetric', 'Location', 'East')
axis([-1 max(DSD_axi.x_start) -1 1])
grid on;


figure(10)
plot(DSD.ipc.px_middle, DSD.ipc.cp_plot); hold on;
plot(DSD_axi.ipc.px_middle, DSD_axi.ipc.cp_plot); grid on;
xlabel('x/c'); ylabel('C_p')
legend('Planar Flow', 'Axi-Symmetric Flow')

%% Now verify mass conservation
[a01, m01, p01] = DSD_axi.compute_induction_on_cross_section(DSD_axi.VS.x_center(1), DSD_axi.VS.y_center(1), 0)
[a10, m10, p10] = DSD_axi.compute_induction_on_cross_section(DSD_axi.VS.x_center(10), DSD_axi.VS.y_center(10), 0)
[a20, m20, p20] = DSD_axi.compute_induction_on_cross_section(DSD_axi.VS.x_center(20), DSD_axi.VS.y_center(20), 0)
[a30, m30, p30] = DSD_axi.compute_induction_on_cross_section(DSD_axi.VS.x_center(30), DSD_axi.VS.y_center(30), 0)
[a40, m40, p40] = DSD_axi.compute_induction_on_cross_section(DSD_axi.VS.x_center(40), DSD_axi.VS.y_center(40), 0)
[a50, m50, p50] = DSD_axi.compute_induction_on_cross_section(DSD_axi.VS.x_center(50), DSD_axi.VS.y_center(50), 0)
...
%     % Print
%     print -dpng -r300 kirikou_example_case_10_velocity_field.png
% end
% 
...
% print -dpng -r300 kirikou_example_case_10_centerline_speed.png
% 
...
% print -dpng -r300 kirikou_example_case_10_induction_breakdown.png
% 
% 
...
% print -dpng -r300 kirikou_example_case_10_cp_distribution.png
% 
% 
