classdef inviscid_panel_case_multielement_axi < handle
    ...
    methods
        function ipc = inviscid_panel_case_multielement_axi(px , py , te_first_indices_input , te_last_indices_input)
            ipc.px = px;
            ipc.py = py;
...
%             end
            ...
            for n_element = 1:ipc.N_elements
                ...
                for n_panel = ipc.te_first_indices_input(n_element):(ipc.te_last_indices_input(n_element)-1)
                    panel_list{n_panel_abs} = panel_axi(ipc.px(n_panel) , ipc.py(n_panel) , ...
                        ipc.px(n_panel+1) , ipc.py(n_panel+1) , []);
...
%                 u_v_t_concatenation_last  = sum(u_v_t_line_last , 2);
        ...
        function post_process(ipc)
            % Post process lifting problem (with no inflow perturbations
            % from other elements)
            ipc.post_process_coupled(ipc.u_inf_x_vec, ipc.u_inf_y_vec)
        end
...
%% These functions are for postprocessing and interaction with other solvers
        ...
        function post_process_coupled_2d(ipc, u_inf_x_vec_bis, u_inf_y_vec_bis)
            % Make sure we use the current forcings
            % Rewrite forcings into normal and tangential components
            [ipc.u_inf_n_vec] = normal_speeds_matrix(ipc.aps , u_inf_x_vec_bis , u_inf_y_vec_bis);
            [ipc.u_inf_t_vec] = tangential_speeds_matrix(ipc.aps , u_inf_x_vec_bis , u_inf_y_vec_bis);
...
%             ipc.L_Cp = norm([ipc.F_x , ipc.F_y]);
        ...
        function post_process_coupled_axi(ipc, u_inf_x_vec_bis, u_inf_y_vec_bis)
            % Make sure we use the current forcings
            % Rewrite forcings into normal and tangential components
            [ipc.u_inf_n_vec] = normal_speeds_matrix(ipc.aps , u_inf_x_vec_bis , u_inf_y_vec_bis);
            [ipc.u_inf_t_vec] = tangential_speeds_matrix(ipc.aps , u_inf_x_vec_bis , u_inf_y_vec_bis);
...
%             s_column = cumsum(l_column);
             ...
             [~, r_c_column] = ipc.aps.centerpoint_locations_columns();
%             % Compute effect of loading 
%             F_x_bis = trapz(s_column, f_x .* (2 * pi* y_c_column))
%             F_y_bis = trapz(s_column, f_y .* (2 * pi* y_c_column))
...
%                 ipc.F_x(n_element) = sum(f_x(element_indices) .* l_column(element_indices));
%                 ipc.F_y(n_element) = sum(f_y(element_indices) .* l_column(element_indices));
                % Switch to trapezoidal integration
                s_column = cumsum(l_column(element_indices));
                % Get panel locations (approx)
                ipc.F_x(n_element) = trapz(s_column, f_x(element_indices) .* (2 * pi* r_c_column(element_indices)));
                ipc.F_y(n_element) = trapz(s_column, f_y(element_indices) .* (2 * pi* r_c_column(element_indices)));
            end
            ipc.L_Cp = -sin(ipc.alpha) * ipc.F_x + cos(ipc.alpha) * ipc.F_y;
            ipc.D_Cp =  cos(ipc.alpha) * ipc.F_x + sin(ipc.alpha) * ipc.F_y;
        ...
        function post_process_coupled(ipc, u_inf_x_vec_bis, u_inf_y_vec_bis)
            % Legacy wrapper
            ipc.post_process_coupled_axi(u_inf_x_vec_bis, u_inf_y_vec_bis);
        end
