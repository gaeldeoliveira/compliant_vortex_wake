classdef panel_axi < handle
    ...
    methods
        function p = panel_axi(x1, y1 , x2, y2 , u_n_BC)
            % Class creator
...
%         p.yc = (p.y1 + p.y2) / 2;
        ...
        function [u_t, u_n] = rewrite_global_vector_in_local_frame(p, u_x, u_y)
            % Create local vector by linear combination of global base
            % elements            
...
%             u_n = beta12 / (2*pi());
            ...
            [u_x, u_y] = p.unit_source_induced_speeds_global_frame(x ,y);
            
            % Convert to Global frame of reference
            [u_t, u_n] = p.rewrite_global_vector_in_local_frame(u_x, u_y);
        end
...
%             [u_x, u_y] = p.rewrite_local_vector_in_global_frame(u_t, u_n);
              ...
              % Compute induction of source segment over panel with unit
              % strenght, in global frame
              % [potential , u , v] = constant_strenght_source_segment_element_axi(x, y, x1, y1, x2, y2, gamma)
            [~ , u_x , u_y] = induction_functions_axi.constant_strenght_source_segment_element_axi(x, y, p.x1, p.y1, p.x2, p.y2, 1);
        end
...
%             u_n = 1/ (2*pi) * log(r2 ./ r1);
            ...
            % Get Induced Speed Vector in Panel frame of reference
            [u_x, u_y] = unit_vortex_induced_speeds_global_frame(p, x ,y);
...
%             [u_x, u_y] = p.rewrite_local_vector_in_global_frame(u_t, u_n);  
            ...
            % Compute induction of vortex segment over panel with unit
            % strenght, in global frame
            % [potential , u , v] = constant_strenght_vortex_segment_element_axi(x, y, x1, y1, x2, y2, gamma)
            [~ , u_x , u_y] = induction_functions_axi.constant_strenght_vortex_segment_element_axi(x, y, p.x1, p.y1, p.x2, p.y2, 1);
        end
