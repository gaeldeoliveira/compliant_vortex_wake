...
%% Inputs
% Geometry
airfoil_file = 'airfoils/naca0012_P320T10.air';  % Filename: Xfoil Plain Format (counterclockwise ordering, TE->LE->LE, adimensionalized by chord)
px_c         =  1/4;            % Airfoil Rotation Point (x coordinate, in airfoil units)
py_c         =  0;              % Airfoil Rotation Point (y coordinate, in airfoil units)
x_duct       = -0.05;           % Translation of airfoil rotation point from origin (x/stream direction)
y_duct       =  60.0;           % Translation of airfoil rotation point from origin (y/radial direction) (works up to 128)
...
%ipc          = inviscid_panel_case_multielement([px ; px + 2] , [py ; py] + 1, [1 ;length(px)+1], [length(px) ;2*length(px)]);
ipc          = inviscid_panel_case_multielement([px_top] , [py_top], [1], [length(px_top)]); %#ok<NBRAK>
...
%% Display Results
% Forces of each element in x (streamwise) direction
disp(['---- 2d Planar Flow ----']); %#ok<NBRAK>
disp(['F_x = ' , num2str(ipc.F_x / (c_duct))]);
% Forces of each element in y (crossstream) direction
disp(['F_y = ' , num2str(ipc.F_y / (c_duct))]);

%% Now add path to axisymmetric solver elements
addpath src_axi/                    % Induction functions wrapper
addpath src_axiHS/                  % Solver-specific changes (HS = Hess-Smith)
addpath src_vortex_ring_induction/ % Induction functions

%% And try an axi-symmetric object
%ipc_axi          = inviscid_panel_case_multielement_axi(px_duct , py_duct, i_start , i_end);
%ipc_axi          = inviscid_panel_case_multielement_axi([px ; px + 2] , [py ; py] + 1, [1 ;length(px)+1], [length(px) ;2*length(px)]);
...
% Make coefficients for axi case


%% Display Results
% Forces of each element in x (streamwise) direction
disp(['---- Axi-symmetric Flow ----']); %#ok<NBRAK>
disp(['F_x = ' , num2str(ipc_axi.F_x  / ((c_duct) * (2 * pi() * y_duct)))]);
% Forces of each element in y (crossstream) direction
disp(['F_y = ' , num2str(ipc_axi.F_y  / ((c_duct) * (2 * pi() * y_duct)))]);

%% Compare Pressure distributions
plot(ipc.px_middle    , ipc.cp_plot); hold on;
plot(ipc_axi.px_middle, ipc_axi.cp_plot); grid on;
legend('Planar', 'Axi-symmetric')
xlabel('x/c');
ylabel('Cp');
